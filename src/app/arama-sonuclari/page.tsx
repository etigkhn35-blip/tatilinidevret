...
function pickDefaultImage(kategori?: string, altKategori?: string) {
  // defaultImages yapısı: defaultImages[kategori]?.[altKategori] || defaultImages[kategori]?.__default || "/defaults/fallback.jpg"
...
export default function SearchResultsPage() {
  ...
  // Firestore’dan çekme (temel filtreler sunucu tarafı, ileri seviye filtreler client tarafı)
  useEffect(() => {
    let qRef: any = query(
  ...
  // İleri seviye filtreleri client’ta uygula
  const filtered = useMemo(() => {
    return items
      ...
      .filter((it) => (typeof minPrice === "number" ? (it.ucret || 0) >= minPrice : true))
      .filter((it) => (typeof maxPrice === "number" ? (it.ucret || 0) <= maxPrice : true))
      .filter((it) =>
        keyword
      ...
      // Tarih aralığına basit kontrol (seçiliyse)
      .filter((it) => {
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map((it) => {
              const img =
                it.coverUrl ||
                pickDefaultImage(it.kategori, it.altKategori);
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="mt-3 flex items-center gap-2">
                      ...
                      <Link
                        href={`/teklif-ver?ilanId=${it.id}`}
                        className="flex-1 text-center px-3 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 text-sm"
                      >
                        Teklif Ver
                      </Link>
                    </div>
